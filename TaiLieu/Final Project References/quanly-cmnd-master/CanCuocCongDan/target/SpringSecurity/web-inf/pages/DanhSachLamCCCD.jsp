			    ...
			    <table border="1" class="thongtin">
		            <tbody>
			            <tr>
			                ...
			                <th style="width:5%;">Chi tiết</th>
			                <th>Duyệt</th>
			                <th style="width:6%;" id='duyetALL'><button onclick="chonTatCa(1);" style="color:blue">Chọn tất cả</button></th>
			            </tr>
...
<script>
	...
	function chonTatCa(duyet){
		...
		if(duyet == 1){
			...
			$("#duyetALL").html("<button onclick='chonTatCa(0);' style='color:blue;'>Bỏ <br> tất cả</button>");
			
		} else {
			for(var i= 0; i < row.length; i++){
				document.getElementById("duyet"+i).checked = false;
			}
			$("#duyetALL").html("<button onclick='chonTatCa(1);' style='color:blue;'>Chọn tất cả</button>");
		}
			...
			$.ajax({url: 'duyet-danh-sach',
				...
				data:danhSach,
			  	success: function(result){
			  		window.location = "duyet-danh-sach-lam-can-cuoc-cong-dan";
		        }});
		}
	}
	function duyet(maSo){
