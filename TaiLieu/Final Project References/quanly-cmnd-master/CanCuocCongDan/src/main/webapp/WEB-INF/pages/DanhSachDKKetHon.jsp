						...
						<c:forEach var="dkKetHon" items="${dsDKKetHon}">
					         <tbody>
								<tr>
									...
									<td>
										<button class="btn btn-primary" onclick="boQua('${dkKetHon.soDK}');">Từ chối</button>
									</td>
...
<%-- 					onsubmit='return checkOnSubmit();'> --%>
<!-- 				<div class="row"> -->
<!-- 					<label class='pull-left mt5'> -->
<!-- 						Tìm kiếm: -->
<!-- 					</label> -->
<!-- 					<div class='col-md-4' id='changeInput'> -->
<!-- 						<input type=text -->
<!-- 								placeholder="Tìm kiếm đơn đăng ký kết hôn"  -->
<!-- 								onkeyup = 'kiemTraSo(this.id, this.value);' -->
<!-- 								id='inputTimKiem'  -->
...
<!-- 										</a> -->
<!-- 									</td> -->
<%-- 									<td><button class="btn btn-primary" onclick="boQua('${dkKetHon.soDK}');">Bỏ qua</button></td> --%>
<%-- 									<c:if test="${dkKetHon.trangThai == 0 }"> --%>
<!-- 										<td style="color: red">Chưa xác nhận</td> -->
<%-- 									</c:if> --%>
<%-- 									<c:if test="${dkKetHon.trangThai == 1 }"> --%>
...
<%-- 	</sec:authorize> --%>
	<script>
		function boQua(soDK) {
			...
			if (!checkNull(lyDo)) {
				window.location = 'bo-qua-dk-ket-hon?soDK='+soDK+"&lyDo="+lyDo;
			} else {
