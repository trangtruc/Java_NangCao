			    ...
			    <div class="row radius-01 bg-white">
		            ...
		            <div class="row mt20">
			            ...
			            <div class="col-md-12 mt20 text-center">
			           		<c:if test="${(!thang eq '' || thang != null) && (!nam eq '' || nam != null)  && (maDonVi eq '' || maDonVi == null) }">
			            		<a href='in-thong-ke-cccd-don-vi' class="btn btn-primary" target=_blank>In thống kê</a>
			           		</c:if>
			           		<c:if test="${(!thang eq '' || thang != null) && (!nam eq '' || nam != null) && (!maDonVi eq '' || maDonVi != null) }">
			            		<a href='in-thong-ke-cccd-don-vi?thang=${thang }&nam=${nam }&dv=${maDonVi}' class="btn btn-primary" target=_blank>In thống kê</a>
			           		</c:if>
			           		<c:if test="${(thang eq '' || thang == null) && (!nam eq '' || nam != null) && (!maDonVi eq '' || maDonVi != null) }">
			            		<a href='in-thong-ke-cccd-don-vi?nam=${nam }&dv=${maDonVi}' class="btn btn-primary" target=_blank>In thống kê</a>
			           		</c:if>
			            </div>
		            </div>
			 </div>
...
function thongKeNam(){
	var nam = $("#nam").val();
	var donVi = $("#dv").val();
	window.location = "thong-ke-cccd-don-vi?nam="+nam+"&dv="+donVi;
}
function thongKeThang(){
	var thang = $("#thang").val();
	var nam = $("#nam").val();
	var donVi = $("#dv").val();
	window.location = "thong-ke-cccd-don-vi?thang="+thang+"&nam="+nam+"&dv="+donVi;
}
</script>
</body>
</html>
