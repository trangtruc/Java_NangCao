...
function getXaFocus(id_xa){
	var id_huyen = id_xa.replace("Xa", "Huyen");
	var huyen = "huyen="+$("#"+id_huyen).val();
	$.ajax({url: 'get-xa',
		type: 'GET',
		data: huyen,
	  	success: function(result){
...
//Ham kiem tra so khi nhap vao cua Tin, khong cho nhap vao chu cai
...
//Ham kiem tra xem gia tri no co nghia hay khong cua Tin
function checkNull(val) {
	return (val == null || val.length < 1 || val == 0 || "Không tìm thấy" == val);
}
//Ham lay thong tin CCCD cua Tin sama
...
function thongBao(id,msg){
	$("#"+id).html(msg);
}
function ngayThangNam(namThangNgay){
	var ngayThangNam = namThangNgay;
	var data = namThangNgay.split("-");
	if(data[0].length == 4){
		ngayThangNam = data[2]+"-"+data[1]+"-"+data[0];
	}
	return ngayThangNam;
}
function inPage(){
...
//Kiem tra do dai cua so dau vao la 9 hoac 12
function ktDoDai(val) {
	return ((replaceAll(val, "-", "").length == 12) || (replaceAll(val, "-", "").length == 9));
}
...
//Tin viet, Kiem tra ngay sinh phai nho hon ngay hien tai
function ktNgaySinh(ngaySinhInput) {
	//Lay ngay hien tai
	var dt = new Date();
	var nam = dt.getFullYear();
	var thang = (dt.getMonth() + 1);
	var ngay = dt.getDate();
	//Lay ngay sinh 
	var d = new Date(ngaySinhInput);
	if (d.valueOf()) { // Valid date
	    namSinh = d.getFullYear();
	    thangSinh = (d.getMonth() + 1);
	    ngaySinh = d.getDate();
	    if (namSinh < nam) {
	    	return true;
	    } 
